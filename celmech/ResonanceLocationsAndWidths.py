...
def resonant_period_ratios(min_per_ratio,max_per_ratio,order):
	...
	msk = np.array( map( lambda x: min_per_ratio < x[0]/float(x[1]) < max_per_ratio , res_ratios ) )
...
def resonance_jk_list(min_per_ratio,max_per_ratio,order):
	...
	msk = np.array( map( lambda x: min_per_ratio < float(x[0]-x[1])/float(x[0]) < max_per_ratio , res_jk ) )
	return res_jk[msk]
