...
class LaplaceLagrangeSystem(Poincare):
    ...
    eccentricity_matrix : sympy.Matrix
        The matrix :math:`\pmb{S}_e` appearing in the secular equations of
        motion for the eccentricity variables,

        .. math::
            \frac{d}{dt}(\eta_i + i\kappa_i) = [\pmb{S}_e]_{ij} (\eta_j +
            i\kappa_j)~.
    ...
    inclination_matrix : sympy.Matrix
        The matrix :math:`\pmb{S}_I` appearing in the secular equations of
        motion for the inclination variables, 
...
class SecularSystemSimulation():
    """
    A class for integrating the secular equations of motion governing a
    planetary system.
    ...
    dtFraction : float, optional
        Set the timestep to a constant fraction of the period of the
        shortest-period linear secular eigenmode.

    max_order : int, optional
        The maximum order of disturbing function terms to include in the
        integration. By default, the equations of motion include terms up to 4th
        order.

    method : str
        Integration method to use. Options include - 'RK':  Direct integration
        of equations of motion using a Runge-Kutta integration method. -
        'splitting': Applies splitting to the flow generated by the system's
        Hamiltonian.

    resonances_to_include : dict, optional
        A dictionary containing information that sets the list of MMRs for which
        the secular contribution will be accounted for (at second order on
        planet masses). Dictionary key and value pairs are specified in the form

        .. code::

          {(iIn,iOut) : [(j_0,k_0),...,(j_N,k_N)]}
        
        This will include the resonances :math:`j_i : j_i-k_i` with
        :math:`i=0,...,N` between planets iIn and iOut. Note that harmonics
        should **NOT** be explicitly included. I.e., if (2,1) appears in the
        list [(j_0,k_0),...(j_N,k_N)] then the term (4,2) should **NOT** also
        appear; these terms' contributions will be added automatically when the
        (2,1) term is added. By default, no MMRs are included.

    rk_kwargs : dict, optional
        Keyword arguments that determine details of the Runge-Kutta scheme used
        to integrate equations of motion. Keywords include

            - :code:`rtol`: float 
                Relative tolerance of root-finding step in the implicit Runge-Kutta scheme. Default is set to machine precision.
            - :code:`atol`: float 
                Absolute tolerance of the root-finding step in the implicit Runge-Kutta scheme. Default is 0 so that tolerance is set solely by :code:`rtol` argument.
            - :code:`rk_method`: str
                Runge-Kutta method to use. Available options include:

                    - :code:`'ImplicitMidpoint'`
                    - :code:`'LobattoIIIB'`
                    - :code:`'GL4'`
                    - :code:`'GL6'`
                    - :code:`'ExplicitMidpoint'`
                    - :code:`'RK4'`
